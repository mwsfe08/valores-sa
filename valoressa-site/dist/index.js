...
async function listPublishedNews() {
  const db = await getDb();
  if (!db) return [];
...
// server/routers.ts
var newsInput = z2.object({
  title: z2.string().trim().min(3).max(220),
  category: z2.string().trim().min(2).max(80),
  publishedAt: z2.coerce.date(),
  excerpt: z2.string().trim().min(10).max(500),
  content: z2.string().trim().min(20),
  imageUrl: z2.string().trim().url().optional().or(z2.literal("")),
  imageData: z2.string().optional(),
  isPublished: z2.boolean().default(true)
});
var updateNewsInput = newsInput.extend({ id: z2.number().int().positive() });
...
async function resolveImage(input, slug) {
  ...
  if (!match) {
    throw new TRPCError3({ code: "BAD_REQUEST", message: "Envie uma imagem JPG, PNG ou WebP v\xE1lida." });
  }
  const [, contentType, encoded] = match;
  const buffer = Buffer.from(encoded, "base64");
  if (buffer.byteLength > 8 * 1024 * 1024) {
    throw new TRPCError3({ code: "PAYLOAD_TOO_LARGE", message: "A imagem deve ter no m\xE1ximo 8 MB." });
  }
...
var appRouter = router({
  ...
  news: router({
    ...
    bySlug: publicProcedure.input(z2.object({ slug: z2.string().min(1) })).query(({ input }) => getPublishedNewsBySlug(input.slug)),
    ...
    update: adminProcedure.input(updateNewsInput).mutation(async ({ input }) => {
      ...
      } else if (input.imageUrl !== void 0) {
        updates.imageUrl = input.imageUrl || null;
      }
      return updateNews(input.id, updates);
    }),
    remove: adminProcedure.input(z2.object({ id: z2.number().int().positive() })).mutation(({ input }) => deleteNews(input.id)),
    getById: adminProcedure.input(z2.object({ id: z2.number().int().positive() })).query(({ input }) => getNewsById(input.id))
  })
});
